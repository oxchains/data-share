...
type PatientRecord struct {
	...
	PatientInfo map[string]PatientData `json:patientInfo`
...
func (t *hospitalChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "setShareItemString":
		return t.setShareItemString(stub, args)
...
func (t *hospitalChaincode) addRecord(stub shim.ChaincodeStubInterface, args []string) pb.Response  {
	...
	patientInfoMap := setDefaultPatientInfo(dataItem)
...
func (t *hospitalChaincode) setShareItem(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var patientInfoMap map[string]PatientData
	err = json.Unmarshal([]byte(patientDataString), &patientInfoMap)
	if err != nil {
		return shim.Error("setShareItem failed : " + err.Error())
	}
    if len(patientInfoMap) != len(record.PatientInfo) {
		return shim.Error("the patientInfo dataitem length is not right")
	}

	record.PatientInfo = patientInfoMap
	record.Ingegral = addIngegral(patientInfoMap)
...
func (t *hospitalChaincode) setShareItemString(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	ingegralArray, err := getIngegral(itemIngegral)
	if err != nil {
		return shim.Error("setShareItem failed : " + err.Error())
	}
    itemArray := getDataItem(dataitem)

    err = setShareItem(itemArray, ingegralArray, record.PatientInfo)
	record.ShareFlag = true
	record.Ingegral = addIngegral(record.PatientInfo)
	err = putState(stub, record.RecordId, record)
    if err != nil {
		return shim.Error("setShareItem failed : " + err.Error())
	}
...
//传输需要共享的item及对应所需的积分.
func setShareItem(dataItem []string, itemIngegral []int, patientInfoMap map[string]PatientData) error {

	for i, item := range dataItem {
		ingegral := itemIngegral[i]
        patientData, ok := patientInfoMap[item]
		if !ok {
			return fmt.Errorf("the item %s is not in original dataitem", item)
		}
		patientData.Flag = true
		patientData.Ingegral = ingegral
		patientInfoMap[item] = patientData
	}
	return nil
}

func getIngegral(ingegralString string) ([]int, error) {
	ingegralStringS := strings.Split(ingegralString, DATAITEM_SEP)
	var res []int
	for _, v := range ingegralStringS {
        ingegral, err := strconv.Atoi(v)
		if err != nil {
			return res, err
		}
		res = append(res, ingegral)
	}
	return res, nil
}
...
func setDefaultPatientInfo(dataItem []string) map[string]PatientData{
	patientInfoMap := make(map[string]PatientData)
...
func getPermissionRecord(permissionType string, record PatientRecord) (PatientRecord, error) {
	if permissionType == QUERY_PERMISSION_ALL {
		...
		for k, v := range record.PatientInfo {
			if v.Flag == false {
				delete(record.PatientInfo, k)
			}
		}
		return record, nil
	} else {
...
func addIngegral(patientInfoMap map[string]PatientData) int {
	ingegral := 0
	for _, v := range patientInfoMap {
		if v.Flag == true {
			ingegral += v.Ingegral
		}
	}
